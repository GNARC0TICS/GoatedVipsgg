              ...
              <div className="mb-6">
                <div className="flex items-center gap-3 mb-4">
                  <img
                    ...
                    className="w-8 h-8"
                  />
                  <h4 className="text-xl md:text-2xl font-heading text-white">
                    {mvp.username}
                  </h4>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="mt-6 p-3 rounded-lg bg-[#D7FF00]/10 border border-[#D7FF00]/20">
                  <div className="flex justify-between items-center">
                    <span className="text-[#D7FF00] text-sm font-semibold">
                      All-Time Wagered:
                    </span>
      ...
      {timeframes.map((timeframe) => (
        ...
        >
          <MVPCard
            timeframe={timeframe}
            mvp={
              mvps[timeframe.period as keyof typeof mvps]
                ? {
                    name:
                      mvps[timeframe.period as keyof typeof mvps]?.name || "",
                    username:
                      mvps[timeframe.period as keyof typeof mvps]?.name || "",
                    uid: mvps[timeframe.period as keyof typeof mvps]?.uid || "",
                    wagerAmount:
                      mvps[timeframe.period as keyof typeof mvps]?.wagered[
                    ...
                    wagered: mvps[timeframe.period as keyof typeof mvps]
                      ?.wagered || {
                      today: 0,
                      this_week: 0,
                      this_month: 0,
                    ...
                    rank:
                      (
                        ...
                        (p: MVP) =>
                          p.uid ===
                          mvps[timeframe.period as keyof typeof mvps]?.uid,
                      ) + 1,
                  }
                : undefined
            }
