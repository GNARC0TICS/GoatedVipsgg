...
// Enhanced debug logging with environment and instance awareness
const debugLog = (...args: any[]) => {
  const timestamp = new Date().toISOString();
  const processId = process.pid;
  if (DEBUG) {
    console.log(`[Telegram Bot Debug][PID:${processId}][${timestamp}]`, ...args);
  } else if (args[0]?.includes('Error') || args[0]?.includes('❌')) {
    console.error(`[Telegram Bot Error][PID:${processId}][${timestamp}]`, ...args);
  }
};
...
const initializeBot = async () => {
  ...
  try {
    // Ensure proper cleanup of any existing bot instance
    if (bot) {
      debugLog("Cleaning up existing bot instance");
    ...
    if (process.env.TELEGRAM_BOT_TOKEN) {
      try {
        debugLog("Clearing existing webhook configuration and pending updates");
        ...
        debugLog("Successfully cleared webhook configuration");
    ...
    debugLog("Initializing new bot instance with polling");
    ...
    debugLog(`Bot successfully initialized and polling started. Username: @${botInfo.username}`);
    debugLog(`Bot instance details: PID=${process.pid}, Polling=true`);
