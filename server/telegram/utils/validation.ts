 ...
 */
export function validateText(text: string | undefined | null, error = 'Text is required'): string {
 ...
 */
export function getCommandParameter(match: RegExpExecArray | null, index = 1): string | undefined {
...
export function containsPotentialInjection(input: string): boolean {
  ...
  // Check all patterns
  return [...sqlPatterns, ...noSqlPatterns].some(pattern => pattern.test(input));
}