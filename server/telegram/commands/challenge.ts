 ...
 */
const CREATE_CHALLENGE_PATTERN = /^\/create_challenge\s+([^|]+)\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*([^|]*)\s*\|\s*(.+)$/;
...
const createChallengeCommand: CommandDefinition = {
  ...
  handler: async (bot: TelegramBot, msg: TelegramBot.Message, match: RegExpExecArray | null): Promise<void> => {
    ...
    try {
      ...
      const [, game, multiplier, minBet, prizeAmount, maxWinnersStr, timeframe, description] = match;
      
      const maxWinners = parseInt(maxWinnersStr.trim());
      if (isNaN(maxWinners) || maxWinners < 1) {
        await bot.sendMessage(chatId, '❌ Max winners must be a positive number.');
        return;
      }
  ...
  handler: async (bot: TelegramBot, msg: TelegramBot.Message): Promise<void> => {
    ...
    try {
      ...
      if (activeChallenges.length === 0) {
        await bot.sendMessage(chatId, 'No active challenges at this time. Check back later!');
        return;
      }
...
const viewChallengeCommand: CommandDefinition = {
  ...
  handler: async (bot: TelegramBot, msg: TelegramBot.Message, match: RegExpExecArray | null): Promise<void> => {
    ...
    if (!match || !match[1]) {
      await bot.sendMessage(chatId, 'Please specify a challenge ID. Usage: /challenge ID');
      return;
    }
