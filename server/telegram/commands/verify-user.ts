...
const verifyUserCommand: CommandDefinition = {
  ...
  handler: async (bot: TelegramBot, msg: TelegramBot.Message, match: RegExpExecArray | null): Promise<void> => {
    ...
    if (!match || !match[1]) {
      await bot.sendMessage(chatId, 'Please specify a Telegram ID to verify. Usage: /verify_user <telegram_id>');
      return;
    }
    ...
    try {
      ...
      if (request.length === 0) {
        await bot.sendMessage(chatId, `❌ No verification request found for Telegram ID: ${telegramId}`);
        return;
      }
      ...
      if (verificationRequest.status === 'approved') {
        await bot.sendMessage(chatId, `✅ This user is already verified by ${verificationRequest.verifiedBy || 'an admin'} on ${verificationRequest.verifiedAt?.toLocaleString() || 'unknown date'}`);
        return;
      }
      ...
      try {
        ...
        await bot.sendMessage(chatId, `Note: Failed to notify the user about their verification.`);
      }
    } catch (error) {
  ...
  handler: async (bot: TelegramBot, msg: TelegramBot.Message): Promise<void> => {
    ...
    try {
      ...
      if (requests.length === 0) {
        await bot.sendMessage(chatId, '✅ No pending verification requests at this time.');
        return;
      }
