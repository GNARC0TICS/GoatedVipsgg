 ...
 */
const VALID_BET_URL_PATTERN =
  /^https?:\/\/(goated\.com|demo\.goated\.gg)\/.*$/i;
...
const verifyEntryCommand: CommandDefinition = {
  ...
  handler: async (
    ...
    if (isNaN(entryId)) {
      await bot.sendMessage(
        chatId,
        "❌ Invalid entry ID. Please provide a valid number.",
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        await bot.sendMessage(
          chatId,
          `⚠️ Note: Failed to notify the user about their verification.`,
        );
      }
    } catch (error) {
...
const reviewEntryCommand: CommandDefinition = {
  ...
  handler: async (
    ...
    if (!match || !match[1]) {
      await bot.sendMessage(
        chatId,
        "Please specify an entry ID. Usage: /review_entry ID",
      );
      return;
    }
