    ...
    if (Array.isArray(rawData.data) && rawData.data.length > 0) {
      ...
      console.log("\nSample Entry Structure:", {
        ...
        wagerFields: sampleEntry.wagered
          ? Object.keys(sampleEntry.wagered)
          : [],
        dataTypes: {
      ...
      const stats = {
        ...
        wagerStats: {
          allTime: {
            ...
            avg:
              rawData.data.reduce(
                (sum: number, e: any) => sum + e.wagered.all_time,
                0,
              ) / rawData.data.length,
          },
          weekly: {
            ...
            avg:
              rawData.data.reduce(
                (sum: number, e: any) => sum + e.wagered.this_week,
                0,
              ) / rawData.data.length,
          },
        },
        activeUsers: {
          weekly: rawData.data.filter((e: any) => e.wagered.this_week > 0)
            .length,
          monthly: rawData.data.filter((e: any) => e.wagered.this_month > 0)
            .length,
          allTime: rawData.data.filter((e: any) => e.wagered.all_time > 0)
            .length,
        },
      };
      ...
      const validation = {
        hasInvalidUIDs: rawData.data.some(
          (e: any) => !e.uid || typeof e.uid !== "string",
        ),
        hasInvalidNames: rawData.data.some(
