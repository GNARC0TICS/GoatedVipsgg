  ...
  app.post("/api/verify/goated", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const [existingVerification] = await db
        ...
        .where(
          and(
            eq(schema.goatedVerificationRequests.goatedUsername, goatedUsername),
    ...
    try {
      ...
      const goatedRequests = await db
        ...
        .leftJoin(schema.users, 
          eq(schema.goatedVerificationRequests.platformUserId, schema.users.id)
        )
      ...
      const telegramRequests = await db
        .select({
          ...
          telegramUsername: schema.telegramVerificationRequests.telegramUsername,
        ...
        .from(schema.telegramVerificationRequests)
        .leftJoin(schema.users, 
          eq(schema.telegramVerificationRequests.platformUserId, schema.users.id)
        )
